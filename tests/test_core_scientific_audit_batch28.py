...
# ---------------------------------------------------------------------------
@pytest.mark.xfail(strict=True, reason="R-57: the guard subtracts two magnitudes on two scales")
...
@pytest.mark.xfail(strict=True, reason="R-57: 300 is compared with 26.85")
...
@pytest.mark.xfail(strict=True, reason="R-57: evaluate returns the law's own unit")
...
@pytest.mark.xfail(strict=True, reason="R-57: the relative tolerance is taken on the declared scale")
def test_r57_the_same_physical_pair_gets_the_same_verdict_in_either_declaration():
    ...
    1000 degC and 1000.0000015 degC are 1273.15 K and 1273.1500015 K: 1.5e-6 apart, which is inside
    ...
    _check(
        ...
        | {BoundaryEdge.BOTTOM: Quantity(1273.1500015, K)},
    )
    _check(
        {edge: Quantity(1000.0, "degC") for edge in BoundaryEdge}
        | {BoundaryEdge.BOTTOM: Quantity(1000.0000015, "degC")},
        unit="degC",
    )
...
# ---------------------------------------------------------------------------
@pytest.mark.xfail(strict=True, reason="R-57: the message labels both numbers with the field's unit")
