...
Recorded as strict xfails in commit d9dabf04, each seen failing on its own assertion, before the fix.
"""
...
def test_r48_a_valid_band_on_an_expected_value_in_celsius_is_not_called_negative():
    ...
    # 26.85 degC is 300.0 kelvin exactly, so 300.6 is 0.6 outside a 0.5 kelvin band and 300.1 is inside.
    assert oracle.compare({"T": Quantity(300.6, "kelvin")}, name="oracle").outcome is (
...
def test_r48_the_physically_correct_declaration_does_not_crash_untyped():
    ...
    failed = oracle.compare({"T": Quantity(301.0, "kelvin")}, name="oracle")
    assert failed.outcome is ValidationOutcome.FAIL
    # ADDED while running batch 29's guard mutations, not preregistered. The RATIO of a difference to a
    # spread is invariant under any affine change of scale, so comparing in the canonical unit rather
    # than the expected value's own changes no number this check reports -- mutation B29i showed that by
    # surviving. What it does change is which scale the refusal NAMES, and a residual labelled
    # 'degree_Celsius' when it is a kelvin difference is the mislabelling batch 28 fixed one layer over.
    assert "kelvin" in failed.detail, (
        f"the refusal must name the scale the residual was measured on; got {failed.detail!r}"
    )
...
def test_r48_no_spelling_of_a_half_degree_band_admits_a_273_kelvin_error():
    """ADDED while running batch 29's guard mutations, not preregistered.

    Two rules stand between a half-degree band and a prediction 273 kelvin wrong: the declaration guard,
    which refuses the ambiguous spelling outright, and the spread reader, which converts the unambiguous
    one by the slope. Each single-rule mutation survived -- with the guard in place only ratio-scale units
    reach the comparison, and there the absolute reader and the spread reader agree, the offset being
    zero. So the invariant is asserted over every spelling at once, and the guard mutation that removes
    BOTH rules is the one that has to be killed.
    """
    for tolerance in (
        Quantity(0.5, "delta_degC"),
        Quantity(0.5, "kelvin"),
        Quantity(0.9, "delta_degF"),
    ):
        oracle = _oracle(Quantity(300.0, "kelvin"), tolerance)
        assert oracle.compare({"T": Quantity(573.0, "kelvin")}, name="oracle").outcome is (
            ValidationOutcome.FAIL
        ), f"a 273 kelvin error passed a band declared as {tolerance}"
    with pytest.raises(ScientificValidationError, match="delta_"):
        _oracle(Quantity(300.0, "kelvin"), Quantity(0.5, "degC"))
