...
`tests/mutation_population_v4.py` holds the 639 guard mutations the 2026-09-16 core re-audit's 55
...
def applied() -> tuple[dict[str, str], dict[str, str], tuple[str, ...]]:
    ...
    scan reads 639 entries against the tree. One measurement, one answer, no chance of the two
    disagreeing.
    """
    stale: dict[str, str] = {}
    inert: dict[str, str] = {}
...
def test_no_two_folded_mutations_share_an_id():
    ...
    assert len(ids) == 639, f"{len(ids)} entries; the population the certificate claims is 639"
...
def test_the_population_is_pinned_by_the_certificate_and_its_digests_are_re_derived():
    ...
    assert population.count == 639 and population.definitions_sha256
...
def test_the_closure_reaches_every_suite_the_population_names():
    ...
    would still not contain the suites this round's 639 entries name -- which are exactly the suites
