...
from . import ADAPTER, SOC_DEFINITION, parameter_set
...
def cell_participant(registry, spec, *, parameter_set_name: str, model: str, initial_soc: float,
    ...
    def soc_of(solution) -> float:
        if solution is None:
            return initial_soc
        return initial_soc - float(solution["Discharge capacity [A.h]"].entries[-1]) / c_nom

    def digest() -> str:
        sol = held["solution"]
        if sol is None:
            return hashlib.sha256(f"initial:{pset.digest}:{initial_soc!r}".encode()).hexdigest()
