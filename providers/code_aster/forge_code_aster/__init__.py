...
from engcore.pde.cases import PlaneStressProblem, ThermoelasticPlaneStressProblem
...
def generate_thermoelastic_mesh(problem: ThermoelasticPlaneStressProblem) -> str:
    """TRIA3 mesh with one GROUP_NO per restraint and one GROUP_MA per material region."""
    mesh = problem.mesh
    if getattr(mesh.cell_type, "value", str(mesh.cell_type)) != "triangle":
        raise ProviderRefusal("this adapter writes TRIA3 meshes only")
    xy = np.asarray(mesh.coordinates, dtype=float)
    ...
    for m in problem.materials:
        ...
        covered |= set(ids)
        names.add(name)
        lines += ["GROUP_MA", name, *(f"M{ci + 1}" for ci in ids), "FINSF"]
    if len(covered) != len(cells):
        raise ProviderRefusal("cells without a material region; nothing is defaulted")
    for i, r in enumerate(problem.restraints):
        nodes = sorted({int(n) for f in facets[mesh.facet_indices(mesh.region(r.group))] for n in f})
        lines += ["GROUP_NO", f"RESTR_{i}", *(f"N{n + 1}" for n in nodes), "FINSF"]
    lines += ["FIN", ""]
    return "\n".join(lines)


def generate_thermoelastic_comm(problem: ThermoelasticPlaneStressProblem, tag: str) -> str:
    ...
    mats = [f"mat_{i} = DEFI_MATERIAU(ELAS=_F(E={float(m.youngs_modulus.to('Pa').magnitude)!r}, "
            f"NU={float(m.poisson_ratio.to('dimensionless').magnitude)!r}, ALPHA={float(exp[m.region].coefficient.to('1/K').magnitude)!r}))"
            for i, m in enumerate(problem.materials)]
    affe = ", ".join(f"_F(GROUP_MA='R_{m.region.upper()[:20]}', MATER=mat_{i})" for i, m in enumerate(problem.materials))
    field = ", ".join(f"_F(NOEUD='N{i + 1}', NOM_CMP='TEMP', VALE={float(v)!r})" for i, v in enumerate(temps))
    dof = {(0,): "DX=0.0", (1,): "DY=0.0", (0, 1): "DX=0.0, DY=0.0"}
    restr = ", ".join(f"_F(GROUP_NO='RESTR_{i}', {dof[tuple(sorted(r.components))]})" for i, r in enumerate(problem.restraints))
    return "\n".join([
        f"# Forge execution {tag}", "DEBUT(LANG='EN')", "mesh = LIRE_MAILLAGE(FORMAT='ASTER', UNITE=20)",
        "model = AFFE_MODELE(MAILLAGE=mesh, AFFE=_F(TOUT='OUI', PHENOMENE='MECANIQUE', MODELISATION='C_PLAN'))",
        *mats, f"tfield = CREA_CHAMP(TYPE_CHAM='NOEU_TEMP_R', OPERATION='AFFE', MAILLAGE=mesh, AFFE=({field},))",
        f"chmat = AFFE_MATERIAU(MAILLAGE=mesh, AFFE=({affe},), AFFE_VARC=_F(TOUT='OUI', NOM_VARC='TEMP', CHAM_GD=tfield, VALE_REF={tref!r}))",
        f"load = AFFE_CHAR_MECA(MODELE=model, DDL_IMPO=({restr},))",
        "res = MECA_STATIQUE(MODELE=model, CHAM_MATER=chmat, EXCIT=_F(CHARGE=load))",
        
        "IMPR_RESU(FORMAT='RESULTAT', UNITE=8, RESU=_F(RESULTAT=res, NOM_CHAM='DEPL', NOM_CMP=('DX', 'DY')))",
        "IMPR_RESU(FORMAT='RESULTAT', UNITE=30, RESU=_F(RESULTAT=res, NOM_CHAM='SIEF_ELGA', NOM_CMP=('SIXX', 'SIYY', 'SIXY')))",
        "FIN()", ""])


def parse_element_stress(text: str, cells: int) -> np.ndarray:
    """SIEF_ELGA in RESULTAT format: a header row ``<element> SIXX SIYY SIXY`` then rows ``<gauss point> sxx syy sxy``.

    One Gauss point per TRIA3.  An incomplete or non-finite table is refused, never padded.
    """
    out: dict[int, list[list[float]]] = {}
    current: int | None = None
    for line in text.splitlines():
        parts = line.split()
        if len(parts) == 4 and parts[0].isdigit() and parts[1] == "SIXX":
            current = int(parts[0]) - 1
            continue
        if current is not None and len(parts) == 4 and parts[0].isdigit():
            try:
                out.setdefault(current, []).append([float(x) for x in parts[1:]])
            except ValueError:
                current = None
    if len(out) != cells or set(out) != set(range(cells)):
        raise ProviderRefusal(f"Code_Aster stress table covers {len(out)} of {cells} elements")
    sig = np.array([np.mean(out[k], axis=0) for k in range(cells)])
    if not np.all(np.isfinite(sig)):
        raise ProviderRefusal("Code_Aster stresses are not finite")
    return sig
...
class CodeAsterProvider:
    ...
    def execute_thermoelastic(self, problem: ThermoelasticPlaneStressProblem) -> ProviderExecutionRecord:
        configuration = {"modelisation": "C_PLAN (per unit thickness)", "operator": "MECA_STATIQUE",
                         "thermal_load": "AFFE_VARC TEMP from a nodal NOEU_TEMP_R field, VALE_REF = T_ref, ELAS ALPHA", "stress_output": "SIEF_ELGA (Gauss point, one per TRIA3)"}
        base = ProviderExecutionIdentity.from_content(self.status, adapter_id=ADAPTER[0], adapter_version=ADAPTER[1], problem=problem.to_dict(),
                                                      configuration=configuration, output_request=("displacement", "stress"))
        t0 = time.perf_counter()
        ws = ProcessWorkspace(self.workspace_root)
        files = {"case.comm": generate_thermoelastic_comm(problem, base.digest), "mesh.mail": generate_thermoelastic_mesh(problem)}
        export = "\n".join(["P time_limit 600", "P memory_limit 2048", "P ncpus 1", "P mpi_nbcpu 1", "P mode interactif",
                            "F comm case.comm D 1", "F mail mesh.mail D 20", "F resu result.resu R 8", "F resu stress.resu R 30", "F mess message R 6", ""])
        identity = ProviderExecutionIdentity.from_content(self.status, adapter_id=ADAPTER[0], adapter_version=ADAPTER[1], problem=problem.to_dict(),
                                                          configuration=configuration, inputs=files, output_request=("displacement", "stress"))
        prefix = os.path.dirname(os.path.dirname(self.status.location))
        env = (("HOME", "/tmp"), ("LC_ALL", "C"), ("OMP_NUM_THREADS", "1"), ("PATH", f"{prefix}/bin:/usr/bin:/bin"))
        inv = ProcessInvocation(self.status.location, self.status.executable_digest, self.status.version, ("case.export",),
                                tuple(GeneratedFile(k, v.encode()) for k, v in sorted({**files, "case.export": export}.items())),
                                env, self.timeout_s, ("result.resu", "stress.resu", "message"))
        try:
            t1 = time.perf_counter()
            rec = ws.run(inv)
            t2 = time.perf_counter()
            if not rec.completed or "DIAGNOSTIC JOB : OK" not in rec.stdout_tail:
                return failed(identity, f"run_aster did not finish OK (exit {rec.exit_code}): {rec.stdout_tail[-500:]}", process_digest=rec.digest, artifacts=files)
            u = parse_displacement(ws.read_output("result.resu").decode("utf-8", "replace"), problem.mesh.node_count)
            sig = parse_element_stress(ws.read_output("stress.resu").decode("utf-8", "replace"), len(problem.mesh.cells))
            t3 = time.perf_counter()
        finally:
            ws.cleanup()
        return ProviderExecutionRecord(identity, True, "", arrays={"displacement": ("m", u), "stress": ("Pa", sig)}, artifacts=files,
                                       process_digest=rec.digest, metrics={"generate_s": t1 - t0, "process_s": t2 - t1, "parse_s": t3 - t2,
                                                                           "nodes": problem.mesh.node_count})
