# K3.1 (predictive admission conditioning): superseded by audit CAP-01

Status: **superseded**. The frozen records (docs/milestones/kinetics-k3.1-freeze.md, docs/milestones/kinetics-k3.1-predictive-admission-conditioning-prereg.md) are not edited; they record what the study measured under the model record in force at its freeze. This note records why it no longer re-derives.
...
k31_run.py re-derives K2 truth means over MULTI_CONDITION_IDS (includes C3) and K3 hold-out truth over HOLDOUT_IDS (includes H1). Both boil, so the study cannot be re-derived.
