# K3 (quantified uncertainty, hold-outs H1/H2): superseded by audit CAP-01

Status: **superseded**. The frozen records (docs/milestones/kinetics-k3-quantified-uncertainty-prereg.md, docs/milestones/kinetics-k3-support-boundary-failure.md) are not edited; they record what the study measured under the model record in force at its freeze. This note records why it no longer re-derives.
...
K3 is posed on the K2 posterior (which uses C3) and evaluated on hold-outs H1 and H2. C3 and H1 boil, so neither the posterior nor the H1 comparison can be re-derived. H2 is admitted (tests/domains/kinetics/test_k3_configuration.py asserts both halves).
