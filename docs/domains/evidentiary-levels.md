...
| `CROSS_SOLVER_VALIDATED` | **yes** | `cstr`'s `independent_solver_agreement`, when the pinned `CrossSolverConsensus` verifies independence between the production solve_ivp route and the separately translated ODEPACK/LSODA route |
...
**Five of six are now occupied.** The CSTR verification gate now has a
separately translated ODEPACK/LSODA route whose preprocessing, implementation,
numerical method and backend are distinct from the production solve_ivp route;
the shared physical problem declaration is explicit in the dependency graph,
and `CrossSolverConsensus` decides whether that declared independence is
sufficient before awarding `CROSS_SOLVER_VALIDATED`. Sprint 2 admitted a repository-pinned
...
| *(the whole report)* — native vs ngspice | **earnable later** | `NEEDS.md` §1.8b's suggestion, and defensible: the two paths share `assemble` but not the solve, so agreement would be `CROSS_SOLVER_VALIDATED`. Cost is environmental rather than architectural — the check can only run where a provider is present, so it is `NOT_RUN` on most machines, and `ngspice.py` is where it would have to live. **(revised)** Half-built since the first pass. `electrical/dc_consensus.py` declares both routes, their shared components and the tolerance, and `CrossSolverConsensus.to_check` emits the `ValidationCheck` that carries the level — that is the architecture, done, with the independence argument written down rather than assumed. It is still `earnable later` for two reasons, both unchanged: the provider has to be installed for the comparison to run at all, and the consensus is a statement *between* two results, so no solver's own `validate()` is in a position to make it — which leaves it outside the `ValidationReport` the credibility package reads. **(revised again — now wired)** The seam exists: `mcp/problem.py` accepts a `cross_solver_check` block, runs the external route on the converged circuit and emits the consensus's own check. Measured on the shipped example against a real provider: nine quantities compared, worst relative difference 4.9e-13 against the declared 1e-9. **The level is still withheld, deliberately and permanently in this report**, and that is not a shortfall in the comparison — the agreement is about the *electrical operating point*, and the values an electro-thermal report carries are temperatures no second route computed. Awarding it would let a report whose thermal reference check was NOT_RUN read CROSS_SOLVER_VALIDATED because two circuit solvers agreed about a voltage. A disagreement still FAILs the report. The row stays *earnable later* because what would earn the level **here** is a second route to the temperature, which is a different piece of work from this one. |
...
| `independent_solver_agreement` | **earnable now — DONE** | The production path uses the domain's solve_ivp implementation; the verification path independently translates the raw reactor declaration and executes ODEPACK/LSODA through `scipy.integrate.odeint`. Their dependency manifests share the physical problem declaration but use distinct preprocessing, implementation, numerical-method and backend identities. The check is emitted by `CrossSolverConsensus.to_check`, so agreement alone is not enough: the Core first re-derives whether the declared routes satisfy the required independence dimensions, and only then can the check establish `CROSS_SOLVER_VALIDATED`. If the route is unavailable it is `NOT_RUN`; a disagreement does not earn the level. |
