...
### Batch 55 — I-30 (R-67, R-68, and R-66's population half)

| ID | Status | Commits | Residuals |
|---|---|---|---|
| I-30 | **DONE** | `ccc72f94` (preregistration + 10 strict xfails), this commit | The batch scripts under `benchmarks/core_v4_false_confidence/audit/` stay unpinned **by decision** — they are the historical record of how each batch ran, and the population is what is verified from now on. R-66's freeze-record half (re-deriving a certificate's commit and figures) is batch 56, where the verifier that reads the record is written |

**The round's own evidence, moved inside the area that is verified.** Fifty-four batches wrote 563 guard
mutations and a log for each, and this batch adds twelve of its own. The evidence was real; nothing measured it. R-67 says so in two halves — the
batch guards are not in the population, and a runner that credits any non-zero exit counts a broken import as
a guard firing — and R-68 is the same defect one level down: the certificate pinned the *suites* the harness
runs and not the modules those suites import.

| Claim | Status | How |
|---|---|---|
| none of the batch guards is in the population | **FIXED** | `tests/mutation_population_v4.py` declares all **575** — the 563 of batches 1–54 and this batch's own 12 — as data, `(id, spec, old, new, target_test, expect, note, also, source)`, inside the harness area the certificate digests. Generated by `benchmarks/.../audit/fold_v4_population.py`, which **parses** each script with `ast` rather than importing it: the batch 1–5 scripts run their mutations at import and one still names an absolute path from the machine the round started on |
| two recorded kills no longer apply | **FIXED, and 23 more nobody had noticed** | **25** entries had stopped describing the tree. Each is repointed at the code the rule lives in now, with the reason, the spec it used to name and its old text carried in its note. **B1j and B1k are repointed, not dropped**: the mission's final round expected them to be unrepointable, and both rules — the supplied grid's goodness of fit and its containment — are still there, one line each in the router's disjunction. Dropping a live guard to match an expectation would be the weakening this round exists to remove |
| any failure counts as a kill | **FIXED** | `tools/certification/mutation_v4_runner.py` runs **only** the entry's target test, with `--junitxml`, and `verdict_from_junit` credits a kill only when that **nodeid** carries a `<failure>`. `NOT_COLLECTED`, `ERRORED`, `SKIPPED`, `NO_REPORT`, `TIMEOUT` and `NOT_APPLIED` are named, never counted. The nodeid is read from the XML because a terminal summary proves that a failure happened, not whose |
| the runners mutate the shared checkout | **FIXED** | `require_an_isolated_tree` refuses the repository and anything inside it. A round interrupted between the edit and the restore can no longer leave a mutated source tree behind |
| the harness area misses the helpers its suites import | **FIXED, and derived** | `harness_import_closure` takes the transitive `tests/`-local import closure of the harness `TARGETS` **and** every target test the population names; `harness_pinning_problems` refuses a scope that does not cover it, and `build_manifest` raises rather than writing one. The area grew from **49** to **123** files. The fix is the derivation: a hand-maintained list is the finding |
| a copied population sha passes every check | **FIXED for the population** | `v4_population` recomputes `population_sha256` over the ordered ids and `definitions_sha256` over the canonical JSON of the **full entries**, so the same ids with a rewritten target test or expectation are a different population; `v4_log_problems` re-derives each shard's verdicts from its own transcript and requires each to equal what the **entry** declares |
| the population stays honest | **FIXED** | `tests/test_mutation_population_v4.py` applies all 575 entries to the live source in memory on every FAST run: the target FUNCTION must exist, not only its file,: exactly one match inside the declared scope, and executable code changed. It is what turns "the population went stale" into a failure in the commit that moved the code |

**Two entries are never applied, and say so.** B25j edits a JSON file (the harness reports MUTATION BROKE THE
PARSE) and B27g edits only prose (MUTATION CHANGED NO CODE). Their batches declared them NOT MUTATED with
their reasons and excluded them from their own runs; both are carried, marked `NOT_MUTATED`, and never
applied. **Five** entries — B31f, B32e, B34b, B47e, B48e — change executable tokens only on Python 3.12+
(PEP 701: before it an f-string was one dropped STRING token). Each mutates a refusal **message** into the
text the audit found misleading and is killed by a test that reads the message, so they are declared in
`FSTRING_ONLY_ON_3_12` and the suite asserts the set is exactly those five.

**Not folded into `tests/mutation_guards.py`, and why.** `MUTATIONS` entries are five-tuples with no target
test, and its runner's kill rule is "the three TARGETS suites went red". Folding 563 entries there would
**lose** the per-entry target test, which is the half of R-67 the audit calls the dangerous one. So the
population is a separate module in the same pinned area, with its own runner and its own FAST-tier check, and
`tests/mutation_guards.py` is untouched — its 6 harness tests still pass over unchanged bytes.

**Amendments 1–3.** `repointed_entries` 27 → 25 → 26, `population_count` 563 → 575, `batch_scripts_folded`
55 → 56. Amendment 2 folds this batch's own twelve guards in: leaving them out would put the guards over the
population's machinery exactly where R-67 found the other 563. Amendment 3 records what the formal round
found — B17f's renamed target test, and four entries whose target tests are **parametrized**, which pytest
reports as `name[case]`; one failing case is the kill, and an entry naming one case explicitly still matches
only that case. Amendment 1: The first measurement counted 27 entries whose `old` does not
match the tree exactly once; two of them are the NOT MUTATED placeholders, whose **empty** `old` "matches"
138 459 times in `certification/guard_reach_ledger.json` and 103 528 times in
`src/engcore/scientific/consensus.py`, because `str.count("")` returns `len(text) + 1`. Neither names a
mutation and neither can be repointed. No rule, class or entry changed; the numbers are in the protocol's
`amendment_log`.

**Compatibility.** ADDITIVE ONLY. No `src/engcore` file is touched at all: this batch is certification
control plane and test data. The harness area's digest moves, which is what `tests/test_core_certificate.py`
and the freeze manifests are in the by-design failing set for, and is settled by the V4 round in batch 56.
...
**Verification.** The batch's own two files **23 passed**. FAST tier **7177 passed, 5 skipped, 19 failed** —
