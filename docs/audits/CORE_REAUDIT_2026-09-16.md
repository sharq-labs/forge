...
mutations and a log for each. The evidence was real; nothing measured it. R-67 says so in two halves — the
...
| the population stays honest | **FIXED** | `tests/test_mutation_population_v4.py` applies all 563 entries to the live source in memory on every FAST run: exactly one match inside the declared scope, and executable code changed. It is what turns "the population went stale" into a failure in the commit that moved the code |
...
**Amendment 1.** `repointed_entries` 27 → 25. The first measurement counted 27 entries whose `old` does not
