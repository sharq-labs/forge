...
class ClaimBinding:
    ...
    def __post_init__(self) -> None:
        ...
        # A frozen dataclass does not freeze the dict behind an attribute.
        object.__setattr__(
            self,
            "qualifiers",
...
class Evidence:
    ...
    def __post_init__(self) -> None:
        ...
        # The payload participates in content_hash/record_hash and is copied into
        # ScientificBelief. Frozen here, or a nested dict/list could change after
        # an admission was signed while the stored hash describes the old content.
    ...
    def require_integrity(self) -> None:
        ...
        Freezing prevents ordinary mutation. This closes the boundary for a caller
        who reaches around ``frozen=True`` or a frozen container: ``record_hash``
        and the Gateway re-derive content identity before an existing
        authorization can be reused for different content.
        """
        ...
        payload.update(
            {
                ...
                "metadata": detach(
                    dict(sorted(self.metadata.items(), key=lambda kv: kv[0]))
                ),
            }
        )
        return payload
