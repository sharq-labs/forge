...
class ModelFormEstimate:
    ...
    reason:str
    estimator_method:ModelFormEstimatorKind=ModelFormEstimatorKind.CONSERVATIVE_MAX_EXCESS
    scope:ModelFormScope|None=None

    def __post_init__(self)->None:
        quantity=str(self.quantity).strip()
        if not quantity: raise ValueError("model-form estimate requires quantity")
        units=require_unit(self.units,context="model-form estimate units")
        object.__setattr__(self,"quantity",quantity)
        object.__setattr__(self,"units",units)
        object.__setattr__(self,"status",ModelFormStatus(self.status))
        object.__setattr__(self,"estimator_method",ModelFormEstimatorKind(self.estimator_method))
        ...
        if self.scope is not None:
            if not isinstance(self.scope,ModelFormScope):
                raise ValueError("model-form scope must be ModelFormScope")
            if self.scope.quantity!=quantity or self.scope.units!=units:
                raise ValueError("model-form estimate quantity/units disagree with its scope")
        ...
        if self.empirical_holdout_coverage is not None:
            ...
            if coverage<0 or coverage>1: raise ValueError("empirical holdout coverage must be in [0,1]")
