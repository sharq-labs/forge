...
class ProducerQualification:
    ...
    def __post_init__(self)->None:
        ...
        reviewer=str(self.independent_reviewer_id).strip()
        quantities=tuple(sorted(set(str(q).strip() for q in self.approved_quantities)))
        if not producer or not method or not reviewer or not quantities or any(not q for q in quantities):
            raise ValueError("model-form producer qualification requires producer, method, independent reviewer and approved quantities")
