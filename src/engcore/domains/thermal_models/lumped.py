        ...
        conditions=(
            ...
            RangeCondition(
                ...
                description=(
                    ...
                    "DECLARED `conservative_screen`, so a horizon under the "
                    "floor STILL REFUSES — nothing here can be certified — but "
                    "refuses as a gap in the evidence rather than as a finding "
                    "against the design. What is insufficient is the horizon "
                    "available to establish the downstream claim, not the "
                    "body: this criterion never observed the lumped model "
                    "being wrong, it ran out of evolution before it could "
            ...
            RangeCondition(
                ...
                description=(
                    ...
                    "ON THE NATURAL ROUTE ONLY: Ra = Gr Pr carries it, and "
                    "Re = u L / nu does not, so on the forced route this "
                    "condition is decidable without one and its value does "
                    "not move when one is added. A forced declaration missing "
                    "a Prandtl number is still UNKNOWN overall -- but through "
                    "convection_property_range_utilization, which is 0.6/Pr, "
                    "and convection_conductance_agreement_ratio, which needs "
                    "the correlated coefficient. The asymmetry is deliberate "
                    "and is pinned by "
                    "test_one_missing_fluid_property_does_not_blank_the_others "
                    "and "
            ...
            RangeCondition(
                ...
                description=(
                    ...
                    "when NEITHER route is available, which is the case where "
                    "there is no characteristic length at all and "
                    "biot_number is already UNKNOWN for the same reason. "
                    "WITH ONE ROUTE THIS CONDITION IS SATISFIED AND NO "
                    "CROSS-CHECK WAS PERFORMED: a caller who declared only a "
                    "length, or only a volume and an area, has supplied one "
                    "unambiguous value, the Biot number is computed from it, "
                    "and nothing contradicts it — but nothing confirmed it "
                    "either. Read this condition in `satisfied` as \'no "
                    "declared geometry contradicts another\', never as \'two "
                    "routes were compared\'. Whether a single-route "
                    "declaration should instead be UNKNOWN is a live question "
                    "about how much this model demands, not a defect: "
                    "test_one_route_alone_is_not_a_contradiction pins the "
                    "current answer deliberately."
                ),
            ),
            RangeCondition(
