"""Extended serializable law contract binding laws to assumptions and conditions."""
...
LAW_SYSTEM_SCHEMA=schema_string("scientific_law_system")
...
class ScientificLawSystem:
    ...
    def to_dict(self)->dict[str,Any]:
        return {
            "schema":LAW_SYSTEM_SCHEMA,"law":self.law.to_dict(),
            "checkable_assumptions":[a.to_dict() for a in self.checkable_assumptions],
            "conditions":[c.to_dict() for c in self.conditions],
        }

    @classmethod
    def from_dict(cls,payload:Mapping[str,Any])->"ScientificLawSystem":
        require_schema(payload,LAW_SYSTEM_SCHEMA)
        return cls(
            LawDefinition.from_dict(payload["law"]),
            tuple(CheckableAssumption.from_dict(x) for x in payload.get("checkable_assumptions",())),
